...
def test_base_cost():
    ...
    assert (
        strategy.calculate_usage(
            Message(id=1, text="hi", timestamp=datetime.datetime(2024, 1, 1))
        ).usage
...
def test_character_count():
    ...
    expected = (
        TextMessageUsageStrategy.BASE_COST
        + (TextMessageUsageStrategy.CHAR_COST * len(test_text.replace(" ", "")))
        + (TextMessageUsageStrategy.LENGTH_4_7 * len(test_text.split(" ")))
    )
    assert pytest.approx(
        strategy.calculate_usage(
            Message(id=1, text=test_text, timestamp=datetime.datetime(2024, 1, 1))
        ).usage
...
def test_word_length_multipliers():
    ...
    expected = (
        TextMessageUsageStrategy.BASE_COST
        + TextMessageUsageStrategy.LENGTH_1_3
        + TextMessageUsageStrategy.LENGTH_1_3
        + TextMessageUsageStrategy.LENGTH_4_7
        + TextMessageUsageStrategy.LENGTH_8_PLUS
        + (TextMessageUsageStrategy.CHAR_COST * len(test_text.replace(" ", "")))
    )
    assert pytest.approx(
        strategy.calculate_usage(
...
def test_third_vowel_positions():
    ...
    expected = (
        TextMessageUsageStrategy.BASE_COST
        + (TextMessageUsageStrategy.LENGTH_4_7 * 2)
        + (TextMessageUsageStrategy.THIRD_VOWEL_COST * 2)
        + (TextMessageUsageStrategy.CHAR_COST * len(test_text.replace(" ", "")))
    )
    assert (
        strategy.calculate_usage(
...
def test_length_penalty():
    ...
    expected = (
        TextMessageUsageStrategy.BASE_COST
        + TextMessageUsageStrategy.LENGTH_PENALTY
        + (TextMessageUsageStrategy.CHAR_COST * len(test_text))
        - TextMessageUsageStrategy.UNIQUE_BONUS
    )
    assert (
        strategy.calculate_usage(
            Message(id=1, text=test_text, timestamp=datetime.datetime(2024, 1, 1))
...
def test_unique_word_bonus():
    ...
    expected = (
        TextMessageUsageStrategy.BASE_COST
        + (TextMessageUsageStrategy.LENGTH_1_3 * len(test_text.split()))
        + (TextMessageUsageStrategy.CHAR_COST * len(test_text.replace(" ", "")))
        - TextMessageUsageStrategy.UNIQUE_BONUS
    )
    assert pytest.approx(
...
def test_palindrome():
    ...
    base_cost = (
        TextMessageUsageStrategy.BASE_COST
        + (TextMessageUsageStrategy.LENGTH_4_7 * 4)
        + (TextMessageUsageStrategy.LENGTH_1_3 * 4)
        + (TextMessageUsageStrategy.CHAR_COST * char_count)
        + (TextMessageUsageStrategy.THIRD_VOWEL_COST * 4)
    )
    assert pytest.approx(
        strategy.calculate_usage(
            Message(id=1, text=test_text, timestamp=datetime.datetime(2024, 1, 1))
