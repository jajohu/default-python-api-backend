...
def test_message_usage():
    message_usage = MessageUsage(
        message_id=42, timestamp=datetime.datetime(2024, 1, 1, 0, 0, 0), credits_used=1
    )
...
def test_usage():
    usage = Usage(
        usage=[
            ...
            MessageUsage(
                message_id=43, timestamp=datetime.datetime(2024, 1, 1, 0, 0, 0), credits_used=1
            ),
        ]
    )
...
def test_calculator(mock_report_api):
    ...
    calculator = Calculator(
        Messages(messages=messages), TextMessageUsageStrategy(), ReportUsageStrategy()
    )
...
def test_calculator_report_not_found_fallback(mock_report_api):
    ...
    messages = [
        Message(
            text="report please", timestamp=datetime.datetime(2024, 1, 1), id=1, report_id=report_id
        )
    ]
    calculator = Calculator(
        Messages(messages=messages), TextMessageUsageStrategy(), ReportUsageStrategy()
