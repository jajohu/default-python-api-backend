...
class Calculator:
    def __init__(
        self,
        messages: Messages,
        text_message_strategy: UsageStrategy,
        report_strategy: UsageStrategy,
    ):
        ...
        for message in self._messages.messages:
            match message.type_:
                case MessageType.TEXT_ONLY:
                    ...
                    yield MessageUsage(
                        message_id=message.id, timestamp=message.timestamp, credits_used=usage.usage
                    )
